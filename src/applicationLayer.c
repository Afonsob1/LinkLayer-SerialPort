...
void sendControl(int fd, unsigned char c, const char *file_name, unsigned long long file_size)
{
    ...
    if (llwrite(fd, control_packet, control_DATA_SIZE) == -1)
        exit(0);
                      ...
                      int nTries, int timeout, const char *filename)
{
    ...
    else
    {
        ...
        // printf("sendControl\n");
        size_t bytes_read = 0;
        while ((bytes_read = fread(buffer + 4, 1, MAX_PAYLOAD_SIZE - 4, file)) != 0)
        {
    ...
    free(buffer);
    llclose(appLayer.fileDescriptor, 1, llRole);
}
