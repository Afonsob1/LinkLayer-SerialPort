...
/* used in statistics tests*/
#define ERRO_CABECALHO 0
#define ERRO_ACKS 0
#define ERRO_INFO 0

int erro_cabecalho = ERRO_CABECALHO;
int erro_reply = ERRO_ACKS;
int erro_informacao = ERRO_INFO;

int nacks = 0;
int acks = 0;
int reply_stats = 0;
int timeouts_stats = 0;
...
void send_data(int fd, char *buffer, int length)
{
    ...
    /* add error*/
    if (erro_cabecalho == 0 && ERRO_CABECALHO)
    {
        frame[3] = 0x12;
        erro_cabecalho = ERRO_CABECALHO;
    }
    ...
    /* add error*/
    if (erro_informacao == 0 && ERRO_INFO)
    {
        frame[4 + length / 2] = 0x12;
        erro_informacao = ERRO_INFO;
    }
    erro_informacao--;
    ...
    write(fd, frame, 4 + length + numEsc + 2);
    free(frame);
}
...
void sendACK(int fd, bool reply)
{
    ...
    if (ERRO_ACKS && erro_reply == 0)
    {
        erro_reply = ERRO_ACKS;
        erro_reply--;
        return;
    }
    erro_reply--;
...
void show_statistics()
{
    ...
    printf("Number of nacks - %d\n", nacks);
    printf("Number of replys - %d\n", reply_stats);
...
int llclose(int fd, int showStatistics, LinkLayerRole ll)
{
    ...
    sleep(1); // make sure receiver gets UA before restoring port settings
